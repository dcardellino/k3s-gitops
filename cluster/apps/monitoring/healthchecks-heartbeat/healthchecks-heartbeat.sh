...
if [[ -z "${UPTIMEROBOT_PING_URL}" ]]; then
    printf "%s - Yikes - Missing UPTIMEROBOT_PING_URL environment variable" "$(date -u)"
    exit 0
fi

status_code=$(curl --connect-timeout 10 --max-time 30 -I -s -o /dev/null -w '%{http_code}' "${UPTIMEROBOT_PING_URL}")
